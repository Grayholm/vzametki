...
from src.core.schemas import NoteUpdateText, ProcessMessageRequest
...
async def classify_message(
    payload: ProcessMessageRequest, service: NotesService = Depends(get_notes_service)
) -> dict:
...
async def list_all_notes(
    user_id: int, service: NotesService = Depends(get_notes_service)
) -> dict:
    return await service.list_all_notes(user_id)


@router.get("/{user_id}/{note_id}")
async def get_note_by_id(
    user_id: int, note_id: int, service: NotesService = Depends(get_notes_service)
) -> dict:
    note = await service.get_note_by_id(user_id, note_id)
    if note:
        return note
    return {"note": None, "action": "get_by_id"}
